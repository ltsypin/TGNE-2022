...
globals$DB_PATH <- system.file("extdata", "pd.filtered.tetexpr.sqlite",
                               package="pd.filtered.tetexpr")
...
pd.filtered.tetexpr <- new("NgsExpressionPDInfo",
                    genomebuild="The Genome Build",
                    getdb=getDb,
                    geometry=as.integer(strsplit("1024;768", ";")[[1]]),
                    annotation="pd.filtered.tetexpr")
