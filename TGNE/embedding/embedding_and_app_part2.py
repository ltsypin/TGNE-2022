...
def plot_embedding(expression_df, embedding_df, annotation_df, label_df, clust_alg, phases, palette, n_components=2, n_neighbors=15, title=None, random_state=42, radius=0.01, normalized=True):
    ...
    label_df = arrange_modules(expression_df, label_df, clust_alg, phases) # FIXME maybe
    ...
    sorted_label_df = label_df
        
    # data = expression_df[list(expression_df.columns)[1:]].values
    
    embedding_df['TTHERM_ID'] = expression_df['TTHERM_ID'].values

    print(list(sorted_label_df[f'{clust_alg}_label_{phases}'].values)[0:10])
    
    merge_unsorted = expression_df.merge(embedding_df, on='TTHERM_ID')

    merge_all = sorted_label_df.merge(merge_unsorted, on='TTHERM_ID')

    if list(sorted_label_df['TTHERM_ID'].values) == list(merge_all['TTHERM_ID'].values):
    ...
    if len(merge) != len(sorted_label_df):
        print('LENGTH DIFFERENT', len(merge), len(sorted_label_df))
    else:
        print('LENGTH SAME!')

    if list(merge['TTHERM_ID'].values) != list(sorted_label_df['TTHERM_ID'].values):
...
with open('./colors_1000_1', 'rb') as file:
