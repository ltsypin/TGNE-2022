...
conflicting_directory <- './pd.gpl6759.ndf.gz/'

unlink(conflicting_directory, recursive = TRUE)

seed <- new("NgsExpressionPDInfoPkgSeed",
              ndfFile='../../new_raw_data/GSE11300/GPL6759.ndf.gz',
...
```{r}
install.packages('./pd.gpl6759.ndf.gz/', repos=NULL, type='source')
```

```{r}
library(pd.gpl6759.ndf.gz)
```

```{r}
xysFiles <- list.xysfiles('.')
expression.feature.set <- read.xysfiles(xysFiles, pkgname = 'pd.gpl6759.ndf.gz')
```
...
```{r}
qc.featureset <- read.xysfiles(qc.files, pkgname = 'pd.gpl6759.ndf.gz')
...
bad.featureset <- read.xysfiles(bad.files, pkgname = 'pd.gpl6759.ndf.gz')
